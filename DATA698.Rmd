...
```{r lm-model, echo=F,message=F,warning=F}
library(tidymodels)  
library(timeDate)
library(vip)       
load(file='RData/CB.RData')
...
library(mice)
tempData <- mice(train_weather,m=5,maxit=50,meth='pmm',seed=500)
summary(tempData)
train_weather <- complete(tempData,1)
tempData <- mice(test_weather,m=5,maxit=50,meth='pmm',seed=500)
summary(tempData)
test_weather <- complete(tempData,1)

rf_fit <- rf_spec %>%
  fit(trips ~ .,
    data = data.frame(train_weather)
  )

rf_fit

results_train <- lm_fit %>%
  predict(new_data = train_weather) %>%
  mutate(
    truth = train_weather$trips,
    model = "lm"
  ) %>%
  bind_rows(rf_fit %>%
    predict(new_data = train_weather) %>%
...
rmse_train <- results_train %>%
  group_by(model) %>%
  rmse(truth = truth, estimate = .pred)
rmse_train

rmse_test <- results_test %>%
  group_by(model) %>%
  rmse(truth = truth, estimate = .pred)
rmse_test
...
# cross validation
set.seed(1234)
bike_folds <- vfold_cv(train_weather, strata = gender)
...
rf_res %>%
  unnest(.predictions) %>%
  ggplot(aes(trips, .pred, color = id)) +
  geom_abline(lty = 2, color = "gray80", size = 1.5) +
  geom_point(alpha = 0.5) +
  labs(
    x = "Truth",
...
```{r r model-glmnet, echo=F,message=F,warning=F}

# DATA SPLITTING & RESAMPLING
df <- rbind(train_weather, test_weather) %>%
  dplyr::select(-train)

set.seed(123)
splits      <- initial_split(df, strata = gender)
...
top_models <-
  ...
  show_best("rmse", n = 30) %>% 
  arrange(penalty) 
# the lowest rmse is associated with model 23
data.frame(top_models)
...
lr_best <- 
  ...
  collect_metrics() %>% 
  arrange(penalty) %>% 
  filter(penalty == top_glmnet$penalty) 
lr_best
lr_rmse <- 
  lr_res %>% 
...
```{r r model-random-forest, echo=F,message=F,warning=F}
# set up multi-thread processing based on number of cores
...
```{r compare-glmnet-and-random-forest, echo=F,message=F,warning=F}
bind_rows(rf_rmse, lr_rmse) %>% 
  ggplot(aes(x = trips, y = .pred, col = model)) + 
  geom_abline(lty = 2, color = "blue", size = 1.5) +
  geom_smooth() +
  geom_point(alpha = 0.5) +
  xlim(c(0, 60)) +
  ylim(c(0, 60)) +
...
rmse_tbl <- data.frame(Model = c('glmnet', 'random forest'),
                       RMSE = c(top_glmnet$mean, top_rf$mean)
)
```
...
```{r last-random-forest, echo=F,message=F,warning=F}
last_rf_mod <- 
  rand_forest(mtry = 9, min_n = 8, trees = 1000) %>% 
...
>>>>>>> 6c1df749648a5584bc8ba381b9fd7abb6c1498ae
